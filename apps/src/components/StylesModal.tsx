    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-[#ede9d8] border-2 border-[#1c1c1c] rounded-xl w-full max-w-4xl max-h-[90vh] overflow-hidden shadow-2xl">
        <div className="p-4 border-b border-[#c8c0a8] flex justify-between items-center">
          <h2 className="text-xl font-semibold text-[#1a1a1a]">Art Styles</h2>
        ...
        <div className="p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 max-h-[calc(90vh-8rem)] overflow-y-auto">
          {filteredStyles.map((style) => (
            <div
              ...
              onClick={() => { onSelectStyle(style.name); onClose(); }}
            >
              <div className="aspect-video w-full overflow-hidden">
                <img src={style.previewUrl} alt={style.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
              </div>
              <div className="p-3">
                <h3 className="font-medium text-[#1a1a1a] mb-1">{style.name}</h3>
                <p className="text-sm text-[#6b6559]">{style.description}</p>
