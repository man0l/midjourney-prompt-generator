...
Only use options and parameters that truly enhance the specific prompt. Combine them naturally and don't force them if they don't fit the context. Output should be a clean, natural-sounding prompt that incorporates these elements seamlessly.`,
  ...
  'grok-image': `You are a Grok AI prompt expert. The user wants an IMAGE — describe it as a vivid, detailed image prompt Grok can render. Include subject, art style, lighting, composition, color palette, mood, and camera details. Be spicy-adjacent (direct, witty) where it helps. Output ONLY the image prompt — no explanations.`,

  'grok-spicy-image': `You are a Grok AI prompt expert for SPICY NSFW image mode. The user wants an IMAGE and spicy is ON — be bold, unfiltered, humor-forward, and willing to engage with explicit/NSFW, edgy angles. Describe the image vividly with subject, style, lighting, composition, color, mood, and camera. If the request is NSFW, describe it explicitly as asked — no sanitizing. Keep Grok's direct, unfiltered voice. Output ONLY the image prompt — no explanations or disclaimers.`,

  'grok-spicy': `You are a Grok AI prompt expert for SPICY NSFW-capable mode. Grok's spicy mode is bold, unfiltered, humor-forward, and willing to engage with explicit, adult, NSFW, or controversial angles without hedging or moralizing. Transform the user's idea into a well-crafted spicy prompt: keep the core request, but add framing that tells Grok to be direct, drop corporate disclaimers, lean into humor/wit, and draw on up-to-the-minute context where useful. If the request is NSFW, keep it NSFW — do not sanitize. Include clear output format guidance. Output ONLY the improved prompt — no explanations or disclaimers.`,
...
export const POST: APIRoute = async ({ request }) => {
  ...
  // Grok spicy (and its image variant) go through OpenRouter / ~x-ai/grok-latest
  const isGrokSpicy = toolType === 'grok-spicy' || toolType === 'grok-spicy-image';
  const isGrokImage = toolType === 'grok-image' || toolType === 'grok-spicy-image';
  ...
  try {
    ...
    const USER_MESSAGES: Record<string, string> = {
      ...
      'grok-spicy': `Transform this into a well-crafted Grok SPICY NSFW prompt (bold, humor-forward, unfiltered, NSFW-capable — skip the hedging):\n\n${prompt}\n\nOutput only the improved prompt.`,
      'grok-image': `Transform this into a vivid Grok IMAGE prompt (describe as an image — subject, style, lighting, composition, colors, mood):\n\n${prompt}\n\nOutput only the image prompt.`,
      'grok-spicy-image': `Transform this into a vivid Grok SPICY NSFW IMAGE prompt (bold, unfiltered, NSFW-capable — describe as an image with subject, style, lighting, composition, colors, mood; keep explicit details if requested):\n\n${prompt}\n\nOutput only the image prompt.`,
